    ...
    {bookmarks.map(bmark => (
      ...
      <div onMouseEnter={() => {
        ...
        <Collapse accordion activeKey={activeKey} className="card" bordered={false}>
          <Panel className="card-panel" showArrow={false} header={<a className="bookmark-link" href={bmark.link} target="_blank">{<div className="card-header">{bmark.title}<BookmarkMenu /></div>}</a>} key={bmark.id}>
